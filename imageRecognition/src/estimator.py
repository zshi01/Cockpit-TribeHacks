...
class TfPoseEstimator:
    ...
    def get_human_node_coordinates(npimg, humans):
        ...
        for human in humans:
            for i in range(common.CocoPart.Background.value):
                if i not in human.body_parts.keys():
                    continue
                # only extract nose, right elbow and right wrist node data.
                # also extract right hip, to get a scale multiplier
                if i in [1, 3, 4, 8]:
                    human_node_data.append([human.body_parts[i].x, human.body_parts[i].y])
                    # human_node_data_x.append([i, human.body_parts[i].x * image_w + 0.5])
                    # human_node_data_y.append([i, human.body_parts[i].y * image_h + 0.5])
                    human_node_data_x.append([i, human.body_parts[i].x])
                    human_node_data_y.append([i, human.body_parts[i].y])


                    # print("nose, elbow, wrist:{0}, x coor:{1}".format(i, human.body_parts[i].x * image_w + 0.5))
        return human_node_data_x, human_node_data_y
